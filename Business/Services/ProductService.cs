...
namespace Business.Services
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(ProductRepositoryBase productRepository, CategoryRepositoryBase categoryRepository, ProductOrderRepositoryBase productOrderRepository)
        {
        ...
        public Result Add(ProductModel model)
        {
            try
            {
                ...
                double unitPrice;
                //unitPrice = Convert.ToDouble(model.UnitPriceText.Trim().Replace(",", "."), new CultureInfo("en-US"));
                //unitPrice = Convert.ToDouble(model.UnitPriceText.Trim().Replace(",", "."), CultureInfo.InvariantCulture);
                //if (!double.TryParse(model.UnitPriceText.Trim().Replace(",", "."), NumberStyles.Any, new CultureInfo("en-US"), out unitPrice)) // Türkçe için: CultureInfo("tr-TR")
                ...
                if (!string.IsNullOrWhiteSpace(model.ExpirationDateText))
                    model.ExpirationDate = DateTime.Parse(model.ExpirationDateText, new CultureInfo("en-US"));
        ...
        public Result Delete(int id)
        {
            try
            {
                // Sipariş yönetimi projeye eklendikten sonra ürün silindiğinde ilişkili ürün sipariş kayıtlarının da silinmesi için eklendi
                #region Product Orders Delete
                var productEntity = _productRepository.EntityQuery(p => p.Id == id, "ProductOrders").SingleOrDefault();
                if (productEntity.ProductOrders != null && productEntity.ProductOrders.Count > 0)
                {
                    foreach (var productOrder in productEntity.ProductOrders)
                    {
                        _productOrderRepository.Delete(productOrder, false);
                    }
        ...
        public IQueryable<ProductModel> Query()
        {
            var query = _productRepository.EntityQuery("Category").OrderBy(p => p.Name).Select(p => new ProductModel()
            {
                ...
                ExpirationDateText = p.ExpirationDate.HasValue ? p.ExpirationDate.Value.ToString("yyyy/MM/dd", new CultureInfo("en-US")) : "",
        ...
        public Result Update(ProductModel model)
        {
            try
            {
                ...
                //unitPrice = Convert.ToDouble(model.UnitPriceText.Trim().Replace(",", "."), new CultureInfo("en-US"));
                //unitPrice = Convert.ToDouble(model.UnitPriceText.Trim().Replace(",", "."), CultureInfo.InvariantCulture);
                //if (!double.TryParse(model.UnitPriceText.Trim().Replace(",", "."), NumberStyles.Any, new CultureInfo("en-US"), out unitPrice))
                if (!double.TryParse(model.UnitPriceText.Trim().Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture, out unitPrice))
                    return new ErrorResult("Unit price must be a decimal number!");
        ...
        public Result<List<ProductsReportModel>> GetProductsReport(ProductsReportFilterModel filter = null, PageModel page = null, AppCore.Business.Models.Ordering.OrderModel order = null)
        {
            try
            {
                #region Query
                ...
                //                    ? p.ExpirationDate.Value.ToString("MM/dd/yyyy", new CultureInfo("en-US"))
                //                    : "",
                //                ProductDescription = p.Description,
                //                ProductName = p.Name,
                //                StockAmount = p.StockAmount,
                //                //UnitPriceText = p.UnitPrice.ToString("C2", new CultureInfo("en-US")), // *2
                //                UnitPriceText = "$" + p.UnitPrice.ToString(new CultureInfo("en-US")), // *2
                //                CategoryId = c.Id
                //            };
                ...
                if (filter != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(filter.ExpirationDateBeginText)) // 05/09/2021
                    {
                        ...
                        DateTime expirationDateBegin = DateTime.Parse(filter.ExpirationDateBeginText, new CultureInfo("en-US"));

                        query = query.Where(q => q.ExpirationDate >= expirationDateBegin);
                    }
                    if (!string.IsNullOrWhiteSpace(filter.ExpirationDateEndText))
                    {
                        DateTime expirationDateEnd = DateTime.Parse(filter.ExpirationDateEndText, new CultureInfo("en-US"));

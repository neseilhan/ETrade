...
namespace MvcWebUI.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            //        TotalUnitPriceText = "$" + cGroupBy.Sum(cgb => cgb.UnitPrice).ToString(new CultureInfo("en-US")),
            ...
            // Toplam birim fiyat ve ürün sayısı için 2. yöntem:
            CartGroupByModel totalSum = new CartGroupByModel() // ProductId ve UserId set edilmediği için 0 default değerini alacak
            {
                IsSum = true,
                ProductName = "Total Price",
                TotalUnitPriceText = "$" + cartGroupBy.Sum(cgb => cgb.TotalUnitPrice).ToString(new CultureInfo("en-US")),
                TotalCount = cartGroupBy.Sum(cgb => cgb.TotalCount)
            };
            cartGroupBy.Add(totalSum);
